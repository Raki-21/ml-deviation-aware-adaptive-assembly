...
def deviation_local_bump(amplitude_mm, sigma=0.12):
    """
    Localized deviation near the centre of the profile.

    This is used to test whether the existing
    locator_offset correction behaves correctly.
    """

    return amplitude_mm * np.exp(
        -((s - 0.5) ** 2) / (2.0 * sigma ** 2)
    )